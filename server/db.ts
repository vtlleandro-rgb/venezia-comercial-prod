...
import {
  ...
  InsertVendaUnidade,
  InsertLogComercial,
  InsertCancelamentoReserva,
  InsertPropostaComercial,
} from "../drizzle/schema";
...
export async function listUnidadesStatus() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(unidadesStatus);
}

export async function salvarUnidadeStatus(data: InsertUnidadeStatus) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(unidadesStatus).values(data).onDuplicateKeyUpdate({
    set: { status: data.status },
  });
  return data;
}

export async function listVendasUnidades() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(vendasUnidades);
}

export async function salvarVendaUnidade(data: InsertVendaUnidade) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(vendasUnidades).values(data).onDuplicateKeyUpdate({
    set: {
      ...
      valorSemDocumentacao: data.valorSemDocumentacao,
...
export async function listLogsComerciais(limit = 200) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(logsComerciais).orderBy(desc(logsComerciais.data)).limit(limit);
}

export async function salvarLogComercial(data: InsertLogComercial) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(logsComerciais).values(data);
  return data;
}

export async function listCancelamentosReservas(limit = 200) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(cancelamentosReservas).orderBy(desc(cancelamentosReservas.data)).limit(limit);
}

export async function salvarCancelamentoReserva(data: InsertCancelamentoReserva) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(cancelamentosReservas).values(data);
  return data;
}

export async function listPropostasComerciais(limit = 200) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(propostasComerciais).orderBy(desc(propostasComerciais.dataGeracao)).limit(limit);
}

export async function salvarPropostaComercial(data: InsertPropostaComercial) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
